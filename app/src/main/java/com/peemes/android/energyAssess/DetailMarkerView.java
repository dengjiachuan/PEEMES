...
public class DetailMarkerView extends MarkerView {
    ...
    public DetailMarkerView(Context context,int layoutResource){
        ...
        try{
            ...
            if (e.getY() == 0) {
               textViewYixi.setText("暂无数据");
                ...
                //textViewYixi.setText(concat(e.getY(),"乙烯收率： "));
                //textViewUOMYixi.setText(concat(e.getY(),"单位乙烯综合能耗： "));
            }
            XAxis xAxis = getChartView().getXAxis();
            CustomValueFormatter formatter = (CustomValueFormatter) xAxis.getValueFormatter();
